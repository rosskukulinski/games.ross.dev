 ...
 *
 * Same Worker as Air Hockey; this game's rooms live under `/hole/room/`.
 */
...
export class OnlineTransport implements Transport {
  ...
  private connect(): void {
    const url = toWebSocketUrl(
      resolveServerBase(),
      `/hole/room/${this.code}?name=${encodeURIComponent(this.name)}`
    );
    ...
    socket.addEventListener('message', (event) => {
      ...
      if (msg.t === 'full') {
        this.closedByUs = true;
        this.onError?.('That arena is full — try hosting a new one.');
        return;
      }
      this.onMessage?.(msg);
    });
